...
```sql
PostgresSQL [escola] SELECT a.nome, COUNT(DISTINCT r.id) AS qtd_respostas,
...
```sql
PostgresSQL [escola] SELECT a.nome, COUNT(r.id) AS qtd_respostas, COUNT(m.id) AS qtd_matriculas
    - FROM aluno a
    -        LEFT JOIN resposta r ON r.aluno_id = a.id
    -        LEFT JOIN matricula m ON m.aluno_id = a.id
    - GROUP BY a.nome;
...
resolvidas utilizando subqueries ou LEFTRIGHT JOIN , porém é importante lembrar que os SGBDs
...
---
## a name=parte13MUITOS ALUNOS E O LIMITa
...
```sql
PostgresSQL [escola] SELECT a.nome FROM aluno a
    - ORDER BY a.nome
    - LIMIT 0,5;
+------------------+
 nome             
